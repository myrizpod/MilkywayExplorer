...
public enum ConveyorType implements MachineryType {
    ...
    BASIC("Tapis Roulant Basique", Material.ACACIA_TRAPDOOR, Tech.AUTOMATISATION_ESSENTIALS, "basic_conveyor", null, 1001,
            new DirectList<>("Permet de transporter des ressources"),
            new DirectList<>(new Tuple<>(Ressource.IRON, 1)),
            new SchematicSetting(0,0), 0.1);
    ...
    private final SchematicSetting schematicSetting; //Not useful for conveyors
    private final double speed;


    ConveyorType(String name, Material mat, Tech prerequis, String id, String model, int modelData, DirectList<String> description, DirectList<Tuple<Ressource, Integer>> price, SchematicSetting schematicSetting, double speed) {
        this.name = name;
        this.material = mat;
    ...
    public static Set<Material> getAllConveyorMaterials() {
        Set<Material> allMats = new HashSet<>();
        for(ConveyorType c : ConveyorType.values()) {
            allMats.add(c.material);
        }
        return allMats;
    }
